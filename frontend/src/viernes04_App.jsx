...
// Auto-dismissing message with color support
...
// Modal de Historial de Atenciones (Versión Profesional)
const UserHistoryModal = ({ show, onClose, userId, userName, db, appId }) => {
  ...
  const [activeTab, setActiveTab] = useState('all'); // 'all', 'emergencies', 'telemedicine'
  ...
  useEffect(() => {
    ...
    // 1. Escuchar Emergencias del usuario
    ...
    // 2. Escuchar Telemedicinas del usuario
        ...
        <div className="flex-grow overflow-y-auto p-6 bg-gray-50">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-400">
              ...
              <p className="mt-2">No se encontraron atenciones para este usuario.</p>
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="p-4 flex-grow grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      <div className="text-sm text-gray-500 font-mono flex flex-col justify-end items-start md:items-end">
                        ...
                        {item.type === 'telemedicine' && item.emergencyId && (
                          <span className="text-xs text-orange-500 mt-1">Vinculada a Emergencia</span>
                        )}
                      </div>
                    </div>
                  </div>
...
const App = () => {
  ...
  const [showSystemUserManagement, setShowSystemUserManagement] = useState(false);
  ...
  // ============================================
  // PASO 2: Escuchar emergencias y telemedicinas
  // ============================================
  useEffect(() => {
    ...
    // 1. Listener para emergencias (Directas)
    ...
    const unsubscribeEmergencies = onSnapshot(emergencyRequestsRef, (snapshot) => {
      ...
      console.error("Error al escuchar emergencias: ", error);
      setToast({ message: "Error al cargar emergencias", type: 'error' });
    });

    // 2. Listener para telemedicinas (INCLUYE ESCALADAS)
    ...
    const unsubscribeTelemedicine = onSnapshot(telemedicineRef, (snapshot) => {
      ...
      console.error("Error al escuchar telemedicinas: ", error);
      setToast({ message: "Error al cargar telemedicinas", type: 'error' });
    });
  ...
  }, [activeTab, db, isAuthReady, user, operatorRole, BACKEND_URL, userListRefreshTrigger]);
  ...
  const updateUserIsActiveStatus = async (userIdToUpdate, newIsActiveStatus, userName) => {
  ...
  const updateEmergencyStatus = async (emergencyId, newStatus, userName, userId, requestType) => {
    console.log('🔥 [DEBUG] updateEmergencyStatus START:', {
      emergencyId,
      newStatus,
      requestType,
      userId
    ...
    if (newStatus === 'in-progress' && ['operador', 'supervisor', 'administrador'].includes(operatorRole)) {
      hasPermission = true;
    } else if (newStatus === 'resolved' && ['supervisor', 'administrador'].includes(operatorRole)) {
      hasPermission = true;
    }
    ...
    if (newStatus === 'resolved') {
      try {
        setToast({ message: 'Verificando historia médica...', type: 'info' });
        console.log('🟡 [DEBUG] Iniciando validación de historia médica...');
        
        if (!userId) {
          console.error('❌ [DEBUG] userId es UNDEFINED');
          setToast({ message: '❌ Error Crítico: UserId es undefined.', type: 'error' });
          return;
        }

        const historyRef = doc(db, `artifacts/${appId}/users/${userId}/medicalHistory/${emergencyId}`);
        const historySnap = await getDoc(historyRef);
        
        console.log('📋 [DEBUG] historySnap.exists():', historySnap.exists());

        if (historySnap.exists()) {
          const historyData = historySnap.data();
          const encounterType = historyData.encounterType;
          
          console.log('✅ [DEBUG] Historia encontrada:', {
            encounterType: encounterType,
            fase1Locked: historyData.fase1?.isLocked,
            fase2Locked: historyData.fase2?.isLocked
          });

          if (encounterType === 'emergency_direct' || encounterType === 'telemedicine_escalated') {
            const fase1Locked = historyData.fase1?.isLocked === true;
            const fase2Locked = historyData.fase2?.isLocked === true;
             
            if (!fase1Locked) {
              console.warn('⚠️ [DEBUG] Fase 1 NO bloqueada');
              setToast({ message: '❌ No se puede cerrar: Fase 1 no está bloqueada.', type: 'error' });
              return;
            }
            if (!fase2Locked) {
              console.warn('⚠️ [DEBUG] Fase 2 NO bloqueada');
              setToast({ message: '❌ No se puede cerrar: Fase 2 no está bloqueada.', type: 'error' });
              return;
            }
            console.log('✅ [DEBUG] Ambas fases bloqueadas - Permitiendo cerrar');
          } else {
            console.error('❌ [DEBUG] Tipo de encuentro no válido para cierre:', encounterType);
            setToast({ 
              message: `❌ No se puede cerrar: Tipo de historia '${encounterType}' no requiere cierre de operador o está incompleta.`, 
              type: 'error' 
            });
            return;
          }
        } else {
          console.error('❌ [DEBUG] Historia NO encontrada en la ruta esperada');
          setToast({ 
            message: '❌ No se encuentra historia médica. Verifica que el médico haya completado Fase 1 y Fase 2.', 
            type: 'error' 
          });
          return;
        }
      } catch (error) {
        console.error('❌ [DEBUG] Error verificando historia:', error);
        setToast({ message: 'Error verificando historia.', type: 'error' });
        return;
      }
    }

    try {
      console.log('🟣 [DEBUG] Actualizando estado en Firestore...');
      const collectionName = requestType === 'telemedicine' ? 'telemedicineSessions' : 'emergencyRequests';
      const docRef = doc(db, `artifacts/${appId}/public/data/${collectionName}/${emergencyId}`);

      console.log(`📝 Actualizando en colección: ${collectionName}`);
      ...
      if (newStatus === 'in-progress') updateData.attendedAt = new Date();
      else if (newStatus === 'resolved') updateData.resolvedAt = new Date();

      await updateDoc(docRef, updateData);
      console.log('✅ [DEBUG] Estado actualizado correctamente');
  ...
  const translateStatus = (status) => {
    switch (status) {
      ...
      case 'in-progress': return 'ATENDIDO';
      case 'resolved': return 'CERRADO';
  ...
  const openMedicalHistory = (userId, userName) => {
    setSelectedUserIdForHistory(userId);
    setSelectedUserNameForMedicalHistory(userName);
  ...
  // ============================================
  // ✅ VALIDACIONES Y DEBUG ANTES DE RENDERIZAR
  // ============================================
  ...
  console.log('✅ App renderizando - db: OK, user:', user?.uid || 'NULL');
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="bg-white shadow-sm p-3 flex justify-center space-x-6">
        ...
        {(operatorRole === 'supervisor' || operatorRole === 'administrador') && (
          <button className={`px-4 py-2 rounded-lg font-semibold ${activeTab === 'reportes' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-600 hover:bg-gray-100'}`} onClick={() => setActiveTab('reportes')}>📊 Reportes</button>
        )} 
      </nav>
      ...
      <main className="flex-grow p-6">
                  ...
                  return (
                    <div key={request.id} className={`p-5 rounded-xl shadow-md border-l-8 ${borderColor} ${bgColor}`}>
                      ...
                      <p className="text-gray-800 text-lg font-bold mt-2">
                        Estado: <span className={`uppercase ${request.status === 'pending' ? 'text-red-600' : 'text-orange-600'}`}>
                          ...
                          <>
                            {(request.status === 'pending' || request.status === 'escalated') &&
                             ['operador', 'supervisor', 'administrador'].includes(operatorRole) && (
                              ...
                              >
                                {request.status === 'escalated' ? 'Marcar Atendido' : 'Marcar Atendido'}
                              </button>
                            )}

                            {request.status === 'in-progress' &&
                             ['supervisor', 'administrador'].includes(operatorRole) && (
                              <button
                    ...
                    {filteredUsers.map(userItem => (
                      <tr key={userItem.id} className="border-b hover:bg-gray-100">
                        ...
                        <td className="py-3 px-6 flex flex-wrap gap-2">
                          {operatorRole === 'administrador' && (
                            <button onClick={() => updateUserIsActiveStatus(userItem.id, !userItem.isActive, userItem.name)} className={`font-bold py-1 px-2 rounded-full text-xs ${userItem.isActive ? 'bg-red-500 hover:bg-red-600 text-white' : 'bg-green-500 hover:bg-green-600 text-white'}`}>{userItem.isActive ? 'Desactivar' : 'Activar'}</button>
                          )}
        ...
        {activeTab === 'telemedicina' && ['medico', 'administrador'].includes(operatorRole) && (
        ...
        {activeTab === 'reportes' && ['supervisor', 'administrador'].includes(operatorRole) && (
